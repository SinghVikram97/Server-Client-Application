...
void handleFileRequestsOnMirror1(int count, int connfd, int sockfdMirror1, char * buffer) {
    int n;
    // connfd is for client 
    // sockfdMirror1 is for writing to Mirror1

    // Write to mirror1 the command
    n = write(sockfdMirror1, buffer, strlen(buffer));
    ...
    // Read like a client from mirro1
    // Read the start signal
    char start_signal[15];
    ssize_t bytes_receivedSignal = read(sockfdMirror1, start_signal, 14);
    ...
    // read file size
    off_t file_size;
    if (read(sockfdMirror1, &file_size, sizeof(file_size)) < 0) {
    ...
    while (total_bytes_received < file_size &&
           (bytes_received = read(sockfdMirror1, buffer, 1024)) > 0) {
...
void handleRequestOnMirror2(int count, int connfd, int sockfdMirror2, char * buffer) {
    ...
    // Read from mirror2
    bzero(buffer, 1024);
    n = read(sockfdMirror2, buffer, 1024);
...
int main(int argc, char *argv[]) {
   ...
   if(connect(sockfdMirror1, (struct sockaddr *) &serv_addr_mirror1, sizeof(serv_addr_mirror1))<0){
       printf("Connection failed to Mirror 1\n");
   }

   /* --------  Open Socket to Mirror1 End --------*/

   /* --------  Open Socket to Mirror2  --------*/
   
   // Open socket to Mirror1
   int sockfdMirror2, portnoMirror2;
   struct sockaddr_in serv_addr_mirror2;
   struct hostent *server_mirror2;
   
   portnoMirror2 = atoi(MIRROR2_PORT);
   sockfdMirror2 = socket(AF_INET, SOCK_STREAM, 0);
   
   if(sockfdMirror2 < 0) {
        printf("Error opening socket to mirror2\n");
   }

   server_mirror2 = gethostbyname(SERVER_IP);
   
   if(server_mirror2==NULL){
        printf("No such host \n");
   }
   
   serv_addr_mirror2.sin_family = AF_INET;
   bcopy((char *)server_mirror2->h_addr_list[0], (char *) &serv_addr_mirror2.sin_addr.s_addr, server_mirror2->h_length);
   serv_addr_mirror2.sin_port = htons(portnoMirror2);
   
   if(connect(sockfdMirror2, (struct sockaddr *) &serv_addr_mirror2, sizeof(serv_addr_mirror2))<0){
       printf("Connection failed to Mirror 2\n");
   }

   /* --------  Open Socket to Mirror2 End --------*/
   ...
   while(1){
      ...
      pid_t pid = fork();
      if(pid==0){
            crequest(count, connfd, sockfdMirror1, sockfdMirror2);
            exit(0);
      }else{
        count++;
        close(connfd);
